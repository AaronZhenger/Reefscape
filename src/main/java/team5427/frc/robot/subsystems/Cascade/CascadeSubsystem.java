...
public class CascadeSubsystem extends SubsystemBase {
  ...
  public void periodic() {
    ...
    if (Math.abs(cascadeSetpoint.in(Meters)) < CascadeConstants.kCascadeMinimumHeight.in(Meters)
        || Math.abs(cascadeSetpoint.in(Meters))
            > CascadeConstants.kCascadeMaximumHeight.in(Meters)) {
    ...
    if (Math.abs(pivotSetpoint.getDegrees())
            < CascadeConstants.kCascadePivotMinimumAngle.getDegrees()
        || Math.abs(pivotSetpoint.getDegrees())
            > CascadeConstants.kCascadePivotMaximumAngle.getDegrees()) {
  ...
  public boolean pivotAtGoal() {
    return (Math.abs(inputsAutoLogged.pivotRotation.minus(pivotSetpoint).getDegrees())
        < CascadeConstants.kPivotTolerance.getDegrees());
  }
  ...
  private static class Errors {
    public static Alert pivotConstraint =
        new Alert(
            ...
            AlertType.kError);
    public static Alert cascadeConstraint =
        new Alert(
            "Constraint Violations",
  ...
  private static class Info {
    public static Alert pivotLocked =
        new Alert("Locked Systems", "Cascade Pivot locked.", AlertType.kInfo);
    public static Alert cascadeLocked =
        new Alert("Locked Systems", "Cascade locked.", AlertType.kInfo);
  }
}
